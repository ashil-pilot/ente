...
void main() {
  ...
  test('Search trace context reaches the paged FilesDB summary', () async {
    late FilesDBMaterializationMetrics metrics;
    const trace = FileLoadTrace(
      family: 'searchBase',
      operationID: 41,
      attempt: 2,
      generation: 7,
    ...
    final files = await FileLoadTrace.run(
      trace,
      () => filesDB.getAllFilesFromDB(const {}, dedupeByUploadId: false),
    );

    expect(files, hasLength(20));
    expect(metrics.traceID, trace.traceID);
    expect(metrics.toLogString(), contains('traceId=${trace.traceID}'));
    expect(metrics.toJson()['traceID'], trace.traceID);
  });
